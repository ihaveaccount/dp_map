...
class DoublePendulum(IMappable):
    ...
    def update_params(self, key):
        ...
        param_step_L = 0.1
        param_step_M = 0.1
        # if key == pygame.K_q: self.L1 = max(0.1, self.L1 - param_step_L); changed = True
        # if key == pygame.K_w: self.L1 += param_step_L; changed = True
        # if key == pygame.K_a: self.L2 = max(0.1, self.L2 - param_step_L); changed = True
        # if key == pygame.K_s: self.L2 += param_step_L; changed = True
        # if key == pygame.K_z: self.M1 = max(0.1, self.M1 - param_step_M); changed = True
        # if key == pygame.K_x: self.M1 += param_step_M; changed = True
        # if key == pygame.K_c: self.M2 = max(0.1, self.M2 - param_step_M); changed = True
        # if key == pygame.K_v: self.M2 += param_step_M; changed = True
